    ...
    if (res.statusCode == 200) {
      try {
        ...
        if (data is List<dynamic>) {
          return FormatResponse.formatSongsResponse(data, 'song');
        } else {
          log("Unexpected data type returned from API.");
          return [];
        }
      } catch (e) {
        log('Error in getReco: $e');
        return [];
      }
    } else if (res.statusCode == 401) {
      log('Authentication error: Unauthorized (401)');
      // 적절한 인증 오류 처리 또는 사용자에게 재로그인 요청
      return [];
    } else {
